...
export class IncomingDataProvider {
  ...
  private isValidRhombusUri(data: string): boolean {
    data = this.sanitizeUri(data);
    return !!this.bwcProvider.getBitcoreRhombus().URI.isValid(data);
  }
  ...
  private isValidRhombusAddress(data: string): boolean {
    return !!(
      this.bwcProvider.getBitcoreRhombus().Address.isValid(data, 'livenet') ||
      this.bwcProvider.getBitcoreRhombus().Address.isValid(data, 'testnet')
    );
  }
  ...
  private isValidRhombusPrivateKey(data: string): boolean {
    return !!(data && this.checkRhombusPrivateKey(data));
  }
  ...
  private handleRhombusUri(data: string, redirParams?: RedirParams): void {
    this.logger.debug('Incoming-data: Rhombus URI');
    let amountFromRedirParams =
      redirParams && redirParams.amount ? redirParams.amount : '';
    const coin = Coin.RHOM;
    let parsed = this.bwcProvider.getBitcoreRhombus().URI(data);
    let address = parsed.address ? parsed.address.toString() : '';
    let message = parsed.message;
    let amount = parsed.amount || amountFromRedirParams;
    if (parsed.r) this.goToPayPro(data, coin);
  ...
  private handlePlainRhombusAddress(
    data: string,
    redirParams?: RedirParams
  ): void {
    this.logger.debug('Incoming-data: Rhombus plain address');
    const coin = Coin.RHOM;
    if (redirParams && redirParams.activePage === 'ScanPage') {
      this.showMenu({
        data,
  ...
  public redir(data: string, redirParams?: RedirParams): boolean {
    ...
    if (this.isValidPayProNonBackwardsCompatible(data)) {
      ...
      // Rhombus  URI
    } else if (this.isValidRhombusUri(data)) {
      this.handleRhombusUri(data, redirParams);
      return true;
      ...
      // Plain Address (Rhombus)
    } else if (this.isValidRhombusAddress(data)) {
      this.handlePlainRhombusAddress(data, redirParams);
      return true;
      ...
      // Check Rhombus Private Key
    } else if (this.isValidRhombusPrivateKey(data)) {
      this.handlePrivateKey(data, redirParams, 'rhom');
      return true;
  ...
  public parseData(data: string): any {
    ...
    if (this.isValidPayProNonBackwardsCompatible(data)) {
      ...
      return {
        data,
        type: 'RhombusUri',
        title: this.translate.instant('Rhombus URI')
      };
      ...
      return {
        data,
        type: 'RhombusAddress',
        title: this.translate.instant('Rhombus Address')
      };
  ...
  private checkRhombusPrivateKey(privateKey: string): boolean {
    // Check if it is a Transaction id to prevent errors
    let isPK: boolean = this.checkRegex(privateKey);
    if (!isPK) return false;
    try {
      this.bwcProvider.getBitcoreRhombus().PrivateKey(privateKey, 'livenet');
    } catch (err) {
      return false;
    }
    return true;
