...
export class HomePage {
  ...
  public showReorderRhom: boolean;
  ...
  private openEmailDisclaimer() {
    let message = this.translate.instant(
      'By providing your email address, you give explicit consent to Rhombus to use your email address to send you email notifications about payments.'
    );
    ...
    () => {
      ...
      this.walletsBtc = _.filter(this.wallets, (x: any) => {
        if (
          ...
          this.logger.info('Old Rhombus Copay wallet detected, updating...');
          x.coin = 'rhom';
          x.credentials.coin = 'rhom';
          this.walletProvider
            .recreate(x)
            .then(() => {
              this.logger.info(
                'Old Rhombus Copay wallet detected, updated successfuly'
              );
            })
            .catch(err => {
              this.logger.error('Error updating old rhombus wallet', err);
            });
          return false;
        }
      ...
      this.walletsRhom = _.filter(this.wallets, (x: any) => {
        return x.credentials.coin == 'rhom';
      });
  ...
  public goToWalletDetails(wallet): void {
    if (this.showReorderBtc || this.showReorderBch || this.showReorderRhom)
      return;
    this.events.unsubscribe('finishIncomingDataMenuEvent');
    this.events.unsubscribe('bwsEvent');
  ...
  public reorderRhom(): void {
    this.showReorderRhom = !this.showReorderRhom;
  }
  ...
  public reorderWalletsRhom(indexes): void {
    let element = this.walletsRhom[indexes.from];
    this.walletsRhom.splice(indexes.from, 1);
    this.walletsRhom.splice(indexes.to, 0, element);
    _.each(this.walletsRhom, (wallet, index: number) => {
      this.profileProvider.setWalletOrder(wallet.id, index);
    });
  }
