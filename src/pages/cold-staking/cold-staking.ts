...
export class ColdStakingPage extends WalletTabsChild {
  ...
  constructor(
    ...
    this.rhombusBitcore = this.bwcProvider.getBitcoreRhombus();
  }
  ...
  public learnMore(): void {
    let url = 'https://rhombus.wiki/staking';
    let optIn = true;
    let title = null;
    let message = this.translate.instant(
      'Learn more about cold staking on the Rhombus wiki.'
    );
    let okText = this.translate.instant('Open Rhombus Wiki');
    ...
    this.popupProvider
      .ionicConfirm(
        ...
        this.translate.instant(
          'This means you will stop receiving interest on your RHOM coins.'
        ),
  ...
  private async balanceTransfer(isZap): Promise<any> {
    try {
      ...
      if (isZap) {
        txp.outputs[0].script = this.rhombusBitcore.Script.fromAddress(
          spend_address,
          stake_address
        ).toString();
      }
